...
function IdCollege() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/colleges?name=${inputValue}&id=${inputValue}`);
      if (response.data.length > 0) {
        const selectedCollege = response.data.find(college => college._id === inputValue || college.name === inputValue);
        if (selectedCollege) {
          setCollege(selectedCollege);
          const studentResponse = await axios.get(`http://localhost:5000/api/students?collegeName=${selectedCollege.name}`);
